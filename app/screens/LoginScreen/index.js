...
const instagram = {
  ...
  redirect_url: baseURL + "login-redirect"
};
...
class LoginScreen extends Component {
  ...
  componentWillMount() {
    Cookie.clear().then(() => {
      //  this.setState({ token: null, loginStatus: false }, () => {});
    });
    if (screenHeight === 812) {
  ...
  handleLoginSucces({ token }) {
    ...
    axios.post("http://analysisplusapp.com/api/login", params).then(response => console.log(response));
  }
  loginButtonPress() {
  ...
  render() {
    return (
      <View style={{ flex: 1, backgroundColor: "#152341" }}>
        ...
        <Image
          style={
            this.state.headerX === false ? styles.appLogo : styles.appXLogo
          }
